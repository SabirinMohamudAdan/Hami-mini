...
// // POST create new order - FIXED VERSION WITH ORDER NUMBER
// router.post('/', async (req, res) => {
//   try {
//     console.log('Received order data:', req.body);
...
//     // GENERATE ORDER NUMBER MANUALLY - XAL SAX AH
//     const lastOrder = await Order.findOne().sort({ createdAt: -1 });
//     let orderNumber = 'HM0001'; // Default
    
//     if (lastOrder && lastOrder.orderNumber) {
//       const lastNumber = parseInt(lastOrder.orderNumber.replace('HM', ''));
//       orderNumber = `HM${String(lastNumber + 1).padStart(4, '0')}`;
//     } else {
//       // Haddii aan jirin dalabo hore, isticmaal timestamp
//       orderNumber = `HM${Date.now().toString().slice(-4)}`;
//     }

//     console.log('Generated order number:', orderNumber);

//     const order = new Order({
//       orderNumber: orderNumber, // ← DAR ORDER NUMBER HALKAN
//       items: items.map(item => ({
//         productId: item.productId || item.id,
//         name: item.name,
//         price: item.price,
...
//     // GENERATE ORDER NUMBER MANUALLY - XAL SAX AH
//     const lastOrder = await Order.findOne().sort({ createdAt: -1 });
//     let orderNumber = 'HM001'; // Default
    
//     if (lastOrder && lastOrder.orderNumber) {
//       const lastNumber = parseInt(lastOrder.orderNumber.replace('HM', ''));
//       orderNumber = `HM${String(lastNumber + 1).padStart(1, '0')}`;
//     } else {
      
//       orderNumber = `HM${Date.now().toString().slice(-1)}`;
//     }
